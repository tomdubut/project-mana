...
async function getPage(userId: string, id: string) {
  const { data } = await supabaseAdmin().from('knowledge_pages').select('*').eq('id', id).eq('user_id', userId).single()
  return data
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  if (!await getPage(userId, id)) return apiError('Not found', 404)
  const { error } = await supabaseAdmin().from('knowledge_pages').delete().eq('id', id)
  if (error) return apiError(error.message, 500)
